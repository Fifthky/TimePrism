...
class PyTorchLightningEstimator(Estimator):
    ...
    def train_model(
        ...
        **kwargs,
        ...
        # Check if the user has already provided a ModelCheckpoint callback in the config.
        has_model_checkpoint = any(
            isinstance(cb, pl.callbacks.ModelCheckpoint) for cb in custom_callbacks
        )

        # Only add a default ModelCheckpoint if the user has not defined one already.
        # This prevents the duplicate callback error.
        if not has_model_checkpoint:
            monitor = "train_loss" if validation_data is None else "val_loss"
            default_checkpoint = pl.callbacks.ModelCheckpoint(
                monitor=monitor, mode="min", verbose=True
            )
            all_callbacks = [default_checkpoint] + custom_callbacks
