...
class TimePrismCore(nn.Module):
    """
    Core implementation for TimePrism_iTran.

    Differences from the original TimePrism core:
    - The temporal dimension of each channel is embedded into a single token
      via a linear projection (no DLinear decomposition).
    - A Transformer encoder operates across channel tokens to model
      dependencies between channels.
    - Trend, seasonal and mixture logits heads operate on the attended
      channel tokens, but the probabilistic / WTA loss logic is kept
      EXACTLY the same.
    """
    ...
    def __init__(self, configs):
        ...
        if self.embedding_dim > 0:
            log.info(
                f"[TimePrism_iTran] Channel embedding is ENABLED with dimension {self.embedding_dim}."
            )
            self.channel_embedding = nn.Embedding(
                num_embeddings=self.enc_in, embedding_dim=self.embedding_dim
            )
        else:
            log.info("[TimePrism_iTran] Channel embedding is DISABLED.")

        # --- Intelligent Factorization of Hypotheses (unchanged) ---
        ...
        log.info(
            f"[TimePrism_iTran] Factorized total_hypotheses={total_hypotheses} into "
            f"N={self.n_hypotheses_trend} (trend) and K={self.n_hypotheses_seasonal} (seasonal)"
        )

        # --- Loss Function Configuration (identical to TimePrism) ---
        ...
        # --- iTransformer-style Channel Tokenization ---
        # Each channel's temporal history (and optional time features) is embedded
        # into a single token. Attention is then applied across channels.
        features_to_add = self.num_feat_dynamic_real if self.use_dynamic_features else 0
        log.info(
            f"[TimePrism_iTran] Number of dynamic features added to tokens: {features_to_add}"
        )

        # Length of per-channel vector before projection: seq_len (+ optional time features summary)
        token_input_dim = self.seq_len + features_to_add

        # Internal token dimension (without channel embedding)
        self.token_dim = 128
        self.token_projection = nn.Linear(token_input_dim, self.token_dim)

        # Final token dimension after optionally concatenating channel embeddings
        self.token_dim_with_emb = self.token_dim + self.embedding_dim

        # Multi-head self-attention across channel tokens
        # We keep this lightweight (2 layers, 4 heads) to stay close to the
        # original TimePrism footprint.
        nhead = 4
        num_layers = 2
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.token_dim_with_emb,
            nhead=nhead,
            batch_first=True,
            dim_feedforward=self.token_dim_with_emb * 4,
        )
        self.channel_attention = nn.TransformerEncoder(
            encoder_layer, num_layers=num_layers
        )

        # --- Heads operating on channel tokens (trend, seasonal, mixture logits) ---
        self.pi_decoder = nn.Linear(self.token_dim_with_emb, self.total_components)
        self.trend_head = nn.Linear(
            self.token_dim_with_emb, self.pred_len * self.n_hypotheses_trend
        )
        self.seasonal_head = nn.Linear(
            self.token_dim_with_emb, self.pred_len * self.n_hypotheses_seasonal
        )

        # Cached tensors used by the loss / sampling logic (unchanged)
...
# Inside the TimePrismCore class
    ...
    def forward(self, x: Tensor, time_feat: Tensor, **kwargs) -> Tensor:
        ...
        - Embeds each channel's history into a token.
        - Applies attention across channels.
        - Produces trend / seasonal hypotheses and mixture logits.
        """
        ...
        B, C, T_in = x_permuted.shape

        # Summarize time features over time, then broadcast to channels
        time_feat_summary = time_feat.mean(dim=1)  # (B, F)

        # --- Step 1: Build per-channel vectors over the temporal dimension ---
        # Base token input: concat signal history and (optional) time features summary
        if self.use_dynamic_features:
            time_feat_summary_expanded = time_feat_summary.unsqueeze(1).expand(-1, C, -1)
            token_input = torch.cat([x_permuted, time_feat_summary_expanded], dim=2)
        else:
            token_input = x_permuted  # (B, C, T)

        # Project each channel's history into a token
        tokens = self.token_projection(token_input)  # (B, C, token_dim)

        # --- Step 2: Optionally concatenate channel embeddings ---
        if self.channel_embedding is not None:
            channel_embeds = self.channel_embedding(target_dim_indicator)  # (B, C, emb_dim)
            tokens = torch.cat([tokens, channel_embeds], dim=-1)  # (B, C, token_dim_with_emb)

        # --- Step 3: Attention across channels ---
        # The transformer encoder expects (batch, sequence, d_model) with sequence = channels.
        tokens_attended = self.channel_attention(tokens)  # (B, C, token_dim_with_emb)

        # --- Step 4: Heads on channel tokens ---
        pi_logits_flat = self.pi_decoder(tokens_attended)  # (B, C, N*K)
        ...
        # --- Step 5: Handle Hypothesis Generation and Final Output ---
        trends_flat = self.trend_head(tokens_attended)
        seasonals_flat = self.seasonal_head(tokens_attended)
...
# Inside the TimePrismCore class
    ...
    def forward_for_evaluation(self, x: Tensor, time_feat: Tensor, **kwargs) -> Tuple[Tensor, Tensor]:
        ...
        # --- Step 4: Calculate Logits ---
        pi_logits_flat = self.pi_decoder(tokens_attended) # Shape: (B, C, N*K)
        pi_logits = pi_logits_flat.view(B, C, self.n_hypotheses_trend, self.n_hypotheses_seasonal)

        # --- Step 5: Compute all hypotheses ---
        trends_flat = self.trend_head(tokens_attended)
        seasonals_flat = self.seasonal_head(tokens_attended)
