...
from tsExperiments.models.project_models.TimePrism.lighting_prism import TimePrismLightning
...
class TimePrismSampleForecastGenerator(ForecastGenerator):
    """
    Wrap samples, or all weighted scenarios for evaluation, as GluonTS forecasts.
    """
    @validated()
    def __init__(self, return_all_hypotheses: bool = False):
        self.return_all_hypotheses = return_all_hypotheses
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for batch in inference_data_loader:
            ...
            if self.return_all_hypotheses:
                # Keep the original per-series evaluation semantics. Window
                # selection is handled by make_evaluation_predictions upstream.
                for i, start_date in enumerate(batch[FieldName.FORECAST_START]):
                    hypotheses, probabilities = prediction_net.forward_for_evaluation(
                        **{name: value[i:i + 1] for name, value in inputs.items()}
                    )
                    info = dict(batch["info"][i] or {}) if "info" in batch else {}
                    info["probabilities"] = to_numpy(probabilities)
