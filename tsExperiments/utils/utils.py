...
def split_train_val(dataset_name, grouped_train, n_pred_steps_val, data_path):
    ...
    Args:
        ...
        n_pred_steps_val (int): number of prediction steps for validation
        data_path (str or Path): path to the directory where datasets are stored
    ...
    # The original call was missing the `path` argument, causing it to search in the default wrong location.
    # We now pass the correct `data_path` to ensure it finds the user-uploaded data.
    # We also convert the path string to a Path object to prevent the 'str' object has no attribute 'mkdir' error.
    dataset = get_dataset(dataset_name, regenerate=False, path=Path(data_path))
    # --- End of modification ---
