...
def main(cfg: DictConfig):
    ...
    # Keep use_full_history for TimePrism.
    ...
    if model_name == "timePrism":
        # Preserve exact scenario probabilities within the standard evaluation path.
        predictor.forecast_generator.return_all_hypotheses = True

    if ((cfg.data.train.type == "Gluonts_ds" and cfg.test is True) or 
        (cfg.data.train.type == "financial_data" and cfg.test is True)):
            ...
            if cfg.model.name == "ETS":
                ...
                # All neural models share the same held-out forecast windows.
                num_samples_for_prediction = 100
                log.info(f"Generating forecasts for model {cfg.model.name}")
    ...
    if cfg.model.plot_forecasts:
        if model_name != "ETS" and ("forecasts" not in locals() or "targets" not in locals()):
            forecast_it, ts_it = make_evaluation_predictions(
                dataset=dataset_test,
                predictor=predictor,
                num_samples=cfg.model.params.num_hypotheses,
        ...
        if model_name == "timePrism":
            ...
            plot_prism(
                forecast_dict={
                    "hypotheses": forecasts[0].samples,
                    "probabilities": forecasts[0].info["probabilities"],
                },
                target_entry={"target": targets[0].values.T, "start": targets[0].index[0]},
                metadata=metadata,
                target_dim=target_dim,
                save_path=Path(cfg.paths.output_dir) / "timeprism_top_hypotheses_plot.png",
                top_k=16,
            )
        elif model_name != "ETS":
            ...
            is_mcl_like = cfg.model.name in ["timeMCL", "timePrism"]
