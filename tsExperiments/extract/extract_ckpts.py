...
logs/
    2025-01-15_13-21-31_seed_3141_electricity_timeMCL_4_relaxed-wta_epsilon_0.1_scaler_mean_hist_Full/
        checkpoints/
            epoch=10.ckpt
    2025-01-15_14-45-22_seed_3141_electricity_timeMCL_4_relaxed-wta_epsilon_0.1_scaler_mean_hist_Full/
        checkpoints/
            epoch=15.ckpt

Example output JSON structure:
{
    "seed_3141_electricity_timeMCL_4_relaxed-wta_epsilon_0.1_scaler_mean_hist_Full": "/path/to/checkpoint1.ckpt"
}
"""
...
def parse_folder_name(
    folder_name: str,
) -> Optional[Tuple[str, str, str, str, str, Optional[str]]]:
    """
    Parse a folder name to extract its components. Now handles variable specificities including history mode.

    Args:
        folder_name: String in format:
            'YYYY-MM-DD_HH-MM-SS_seed_${seed}_dataset_model_num_hypotheses[_modelspecificities]'

    Returns:
        Tuple containing (datetime_str, seed, dataset_name, model_name, num_hypotheses, model_specificities)
        where model_specificities includes all parts after num_hypotheses.
        Returns None if the folder name doesn't match expected format.
    """
    try:
        parts = folder_name.split("_")
        if len(parts) < 7:  # A valid name must have at least date, time, seed, ID, dataset, model, num_hyp
            return None

        # Extract date and time parts
        datetime_str = f"{parts[0]} {parts[1].replace('-', ':')}"

        # Extract fixed position parts
        seed = parts[3]

        # --- Special handling for datasets with underscores (currently: uci_air) ---
        # Standard pattern: YYYY-MM-DD HH-MM-SS seed SEED DATASET MODEL NUM_HYP [SPECIFICITIES...]
        # For datasets like 'uci_air', the split produces: ..., 'seed', SEED, 'uci', 'air', MODEL, NUM_HYP, ...
        # We detect this case explicitly to avoid confusing 'air' with the model name.
        if parts[4] == "uci" and parts[5] == "air":
            dataset_name = "uci_air"
            # Model name now starts at parts[6]
            if "timePrism" in parts and "iTran" in parts:
                # Example:
                # ... 'seed', SEED, 'uci', 'air', 'timePrism', 'iTran', NUM_HYP, ...
                model_name = "timePrism_iTran"
                num_hyp_index = 8
            elif "transformer" in parts and "tempflow" in parts:
            ...
            # Handle model name potentially containing underscores
            if "timePrism" in parts and "iTran" in parts:
                # Example:
                # ... 'seed', SEED, DATASET, 'timePrism', 'iTran', NUM_HYP, ...
                model_name = "timePrism_iTran"
                num_hyp_index = 7
            elif "transformer" in parts and "tempflow" in parts:
                model_name = "transformer_tempflow"
        ...
        num_hypotheses = parts[num_hyp_index]

        # All remaining parts constitute the model specificities
        specificities_parts = parts[num_hyp_index + 1:]
        model_specificities = "_".join(specificities_parts) if specificities_parts else None
        ...
        print(f"Could not parse folder '{folder_name}': {e}")
        return None
...
def build_checkpoint_structure(
    ...
    Returns:
        ...
        {
            "seed_${seed}_dataset_model_num_hypotheses[_modelspecificities]": [list of checkpoint paths]
        }
    """
...
def main():
    ...
    parser.add_argument(
        "--datasets",
        type=str,
        default="electricity_200,exchange_200,solar_200,taxi_200,traffic_200,wiki_200,crypt_101,uci_air_200, hospital_200, mdense_200, hierachi_200",
